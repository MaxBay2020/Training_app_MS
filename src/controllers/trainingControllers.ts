...
import TrainingType from "../entities/TrainingType";
...
class TrainingController {
    ...
    static queryAllTrainingTypes = async (req: ExpReq, res: ExpRes) => {
        try {
            const allTrainingTypes: TrainingType[] = await dataSource
                .getRepository(TrainingType)
                .createQueryBuilder('trainingType')
                .select(['trainingType.trng_typ_nm'])
                .where('trainingType.trng_typ_nm <> :trainingType', { trainingType: TrainingTypeEnum.ECLASS })
                .getMany() as TrainingType[]

            const allTrainingTypesArr = allTrainingTypes.map(trainingType => trainingType.trng_typ_nm)

            return res.status(StatusCode.E200).send(allTrainingTypesArr)
        }catch (e) {
            console.log(e.message)
            const error = new Error<{}>(e, StatusCode.E500, Message.ServerError)
            return res.status(error.statusCode).send({
